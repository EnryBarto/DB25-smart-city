...
\begin{itemize}
    ...
    \item Gestione tragitti: nel lato sinistro è possibile aggiungere una nuova tratta alla fine della linea selezionando le due da un menu a tendina. Nel lato destro è possibile rimuovere l'ultima tratta assegnata a una linea.
    \item Gestione orari linee: nel lato sinistro è possibile aggiungere un orario di partenza settimanale ad una linea. Va selezionata la linea, l'ora di partenza e uno o più giorni di validità. Se vengoono selezionati più giorni, questi vengono tutti aggiunti all'orario. Nel lato destro è possibile data una linea, eliminare un orario.
    \item Inserimento attuazione corsa: qui si può aggiungere una corsa al sistema, specificando il giorno, la linea, l'orario di partenza, il mezzo e l'autista. Gli orari di partenza si devono scegliere da una lista di orari predefiniti e sono filtrati in base al giorno settimanale, quindi viene rispettano il vincolo che un'attuazione corsa deve essere effettuata in coerenza all'orario della linea.
    ...
    \item Inserimento variazione di servizio: data una manuntenzione relativa a una linea è possibile aggiungere una linea sostitutiva. La manuntenzione e la linea da sostituire devono essere aggiunte nelle loro schede. Le linee da sostituire possono essere solo straordinarie.
    \item Gestione hub: nel lato sinistro è possibile aggiungere un hub mobilità. Eventualmente si può anche aggiungere una fermata tra quelle presenti nel menu a tendina. In rimozione hub si possono rimuovere degli hub già inseriti nel sistema.
    \item Associazione Hub-Contenuto: nel lato sinistro è possibile associare a un hub un contenuto (macchine elettriche, bici scooter, ecc...) e il numero di posti massimi relativi ad esso. Gli hub devono essere già inseriti nel sistema in \textit{Gestione hub}. Nel lato destro è possibile rimuovere un'associazione.
    \item Assegna controllore: qui si può assegnare un controllore presente nel sistema a un'attruazione presente nel sistema. Si selezionano da un menu a tendina.
    \item Statistiche: in questa scheda sono presenti diverse statistiche riguardanti il sistema. Si seleziona il tipo di statistica da visualizzare in un menu a tendina e si preme \textit{OK} per ricevere il risultato. Per alcune operazioni è necessario aggiungere ulteriori informazioni: si devono compilare i campi e premere il relativo pulsante per ottenere il risultato.
