...
Le obliteratrici disporranno inoltre di un display per mostrare il codice della corsa, così che gli utenti saranno anche in grado di convalidare il proprio biglietto dall'area personale dell'applicativo inserendo questo codice.”
...
Biglietti, Abbonamenti & Titoli di Viaggio \\
Gestore delle linee & Amministrativo \\
\hline
\end{tabular}
\end{table}

A seguito dell’intervista aggiuntiva è stata redatta la seguente descrizione del dominio in linguaggio naturale. Ambiguità e informazioni ridondanti sono state eliminate per garantire una miglior fruizione della descrizione; abbiamo inoltre sottolineato i concetti principali che ci saranno utili per la prossima fase di progettazione concettuale.
\\ \\
...
Un unico titolo di viaggio vale per tutti i mezzi di trasporto e si vuole memorizzarne la durata, la data di convalida, la data di acquisto e l’utente cittadino associato nel caso sia un \underline{\texttt{biglietto digitale}}. Sono presenti diverse \underline{\texttt{tipologie}} di titoli di viaggio che differiscono in base alla durata dalla convalida e al prezzo. Gli abbonamenti sono biglietti che hanno una durata superiore a cinque giorni. I biglietti possono essere acquistati anche in biglietterie fisiche e ticketmachine. Ogni mezzo di trasporto sarà dotato di una obliteratrice che, comunicando con il sistema informativo, alla lettura di un biglietto ne controlla la validità, e in caso affermativo, lo registra come \underline{\texttt{convalidato}}.

Si richiede anche la gestione dei \underline{\texttt{dipendenti}}. Essi si dividono in \underline{\texttt{Autisti}}, \underline{\texttt{Controllori}} e \underline{\texttt{Amministrativi}}. Agli Autisti verranno assegnati diversi mezzi di trasposto in slot orari diversi. I controllori dovranno controllare diverse linee negli slot orari assegnati, e saranno in grado di emettere \underline{\texttt{multe}}.  A ogni \underline{\texttt{causale della multa}} corrisponde un certo importo base e uno massimo. Il controllore potrà assegnare un importo alla multa tra questi due valori. Gli orari e le linee da controllare vengono stabilite dagli amministrativi; questi ultimi loggandosi nel portale potranno gestire autisti, controllori e le linee. Infine, avranno la possibilità di visualizzare diverse statistiche riguardanti il sistema di trasporto, con enfasi particolare nei ricavi dei titoli di viaggio e nel tasso di evasione.
...
\includepdf[landscape=true]{src/pdf/Schema_ER.pdf}
...
Per poter gestire al meglio il carico di lavoro della base di dati, insieme al committente si è creata una stima del volume dei dati per entità e associazioni che il database dovrà gestire, di cui se ne riportano i valori nella Tabella \ref{table:volumeDati}.
La stima è valida per un carico di lavoro di circa 6 mesi.
Abbiamo considerato un bacino d'utenza di circa 20.000 persone.
...
\hline
6 & \hyperref[op6]{Estrazione delle linee con più convalide nell'ultimo mese} & 3 & Amministrativo  \\
\hline
7 & \hyperref[op7]{Estrazione delle manuntenzioni che coinvolgono un determinato mezzo} & 5 & Amministrativo \\
\hline
8 & \hyperref[op8]{Estrazione delle manuntenzioni ed eventuali linee sostitutive che coinvolgono una linea} & 5 & Amministrativo \\
\hline
9 & \hyperref[op9]{Visualizzazione incassi dati dalle convalide per una linea} & 10 & Amministrativo \\
\hline
10 & \hyperref[op10]{Estrazione degli incassi per tipo di titolo in periodo definito} & 5 & Amministrativo \\
\hline
11 & \hyperref[op11]{Estrazione delle linee con più multe in periodo definito} & 2 & Amministrativo \\
\hline
12 & \hyperref[op12]{Estrazione delle 5 linee con manutenzioni più gravose (in termini linee sostitutive e durata)} & 2 & Amministrativo  \\
\hline
13 & \hyperref[op13]{Estrazione delle linee con \textgreater 5 controlli/giorno e \textless = 10 multe/giorno} & 3 & Amministrativo  \\
\hline
14* & \hyperref[op14]{Visualizzazione delle linee con il maggior tempo di percorrenza} & 2 & Amministrativo \\
\hline
15 & \hyperref[op15]{Estrazione della linea con più hub mobilità lungo il percorso} & 3 & Amministrativo \\
\hline
16 & \hyperref[op16]{Media di soldi spesi in multe per persona} & 2 & Amministrativo \\
\hline
17 & \hyperref[op17]{Visualizzazione delle aziende che non hanno effettuato nessuna manutenzione nell'ultimo mese} & 4 & Amministrativo \\
\hline
18 & \hyperref[op18]{Visualizzazione delle fermate in cui è presente un hub mobilità contenente tutti i tipi di servizi green} & 1 & Amministrativo \\
\hline
19* & \hyperref[op19]{Inserimento di una variazione di servizio} & 1 & Amministrativo \\
\hline
20* & \hyperref[op20]{Aggiunta di una tratta a una linea esistente} & 1 & Amministrativo \\
\hline
21* & \hyperref[op21]{Creazione di una nuova linea} & 1 & Amministrativo \\
\hline
\end{longtable}
...
\begin{enumerate}[label=\textbf{\arabic*}]
  ...
  % -------------------------------------------------------------------------------------------------------------------------------------------
    ...
    \hline
    \multicolumn{4}{c}{\textbf{Totale}} \\
    \multicolumn{4}{c}{${A_{lettura}}$ = 55, ${A_{scrittura}}$ = 0} \\
    \hline
    \end{tabular}
    \end{table}
    \begin{center}
    ${C_{tot} = {O_{settimana}}\cdot {A_{lettura}} = 220}$
    \end{center}
...
\hline
6 & \hyperref[op6]{Estrazione delle linee con più convalide nell'ultimo mese} & 850.005 & Amministrativo  \\
\hline
7 & \hyperref[op7]{Estrazione delle manuntenzioni che coinvolgono un determinato mezzo} & 35 & Amministrativo \\
\hline
8 & \hyperref[op8]{Estrazione delle manuntenzioni ed eventuali linee sostitutive che coinvolgono una linea} & 205 & Amministrativo \\
\hline
9 & \hyperref[op9]{Visualizzazione incassi dati dalle convalide per una linea} & 8.150 & Amministrativo \\
\hline
10 & \hyperref[op10]{Estrazione degli incassi per tipo di titolo in periodo definito} & 250.005 & Amministrativo \\
\hline
11 & \hyperref[op11]{Estrazione delle linee con più multe in periodo definito} & 503.700 & Amministrativo \\
\hline
12 & \hyperref[op12]{Estrazione delle 5 linee con manutenzioni più gravose (in termini linee sostitutive e durata)} & 100 & Amministrativo  \\
\hline
13 & \hyperref[op13]{Estrazione delle linee con \textgreater 5 controlli/giorno e \textless = 10 multe/giorno} & 755.100 & Amministrativo  \\
\hline
14* & \hyperref[op14]{Visualizzazione delle linee con il maggior tempo di percorrenza} & 150 & Amministrativo  \\
\hline
15 & \hyperref[op15]{Estrazione della linea con più hub mobilità lungo il percorso} & 9.570 & Amministrativo \\
\hline
16 & \hyperref[op16]{Media di soldi spesi in multe per persona} & 20.000 & Amministrativo \\
\hline
17 & \hyperref[op17]{Visualizzazione delle aziende che non hanno effettuato nessuna manutenzione nell'ultimo mese} & 220 & Amministrativo \\
\hline
18 & \hyperref[op18]{Visualizzazione delle fermate in cui è presente un hub mobilità contenente tutti i tipi di servizi green} & 1.129 & Amministrativo \\
\hline
19* & \hyperref[op19]{Inserimento di una variazione di servizio} & 7 & Amministrativo \\
\hline
20* & \hyperref[op20]{Aggiunta di una tratta a una linea esistente} & 28 & Amministrativo \\
\hline
21* & \hyperref[op21]{Creazione di una nuova linea} & 61 & Amministrativo \\
\hline
\end{longtable}
...
Gli unici attributi multivalore nel nostro schema sono gli \texttt{indirizzi}, e sono presenti nelle entità \texttt{AZIENDA}, \texttt{FERMATA} e \texttt{HUB MOBILITÀ}.
...
\includepdf[landscape=true]{src/pdf/Schema_Logico.pdf}
...
\section{Trigger}\label{section:trigger}
...
Nelle query potrebbero apparire dei simboli \texttt{?}: questo perché abbiamo utilizzato i prepared statement per poter adattare le clausole \texttt{WHERE} ai filtri imposti dagli utenti.
...
Questa operazione è stata divisa in 2 parti: la prima parte serve per ricavare il tragitto di una linea, la seconda per estrarre tutti gli orari di partenza della linea. Così potremo comporre l'orario sommando tutti i tempi di percorrenza dei tragitti a ogni orario di partenza.
\begin{lstlisting}[style=sqlstyle, caption=Query per ricavare i tragitti di una linea dato il codice]
...
\begin{lstlisting}[style=sqlstyle, caption=Query per ricavare tutti gli orari di una linea dato il codice]
SELECT *
FROM ORARI_LINEE
WHERE codice_linea = ?;
\end{lstlisting}
...
Lo scopo è quello di estrarre le linee che in \textunderscore{tutti} i giorni in cui sono state effettuate attuazioni corsa, sono stati svolti 5 controlli e 10 multe.
Nella subquery conto quante sono le attuazioni corsa distinte che, raggruppate in giorni, soddisfano la condizione per una determinata linea.
Alla fine queste vengono confrontate con il numero di attuazioni corsa totali per quella linea. Se sono lo stesso numero (ovvero tutte le attuazioni corsa rispettano la condizione) allora la linea viene visualizzata.
\begin{lstlisting}[style=sqlstyle]
...
In seguito all'aggiunta di una manutenzione bisognerebbe aggiornare l'attributo \texttt{attiva} su \texttt{LINEA}. Nel nostro caso non è però necessario grazie all'automatismo introdotto dal trigger (vedi \cref{section:trigger}).
\begin{lstlisting}[style=sqlstyle]
...
L'app si basa sul patttern MVC, in cui il controller si occupa di tenere attiva la connessione con il DBMS. In particolare le query vengono effettuate tramite DAO inseriti nelle classi del model. Abbiamo usufruito delle classi \texttt{DAOUtils} e \texttt{DAOException} presentateci nelle esercitazioni a lezione.

\subsection{Creazione del DB}
Per la creazione del DB e di tutte le funzionalità aggiuntive (viste, stored procedures, ecc...) abbiamo messo a disposizione un unico file chiamato \texttt{creazione\_db.sql}. Dopo aver creato il DB è necessario inserire alcuni dati di base, utilizzando il file \texttt{required\_data\_insertion.sql}. Quest'ultimo contiene anche un account amministrativo di credenziali User: \texttt{admin} e Password: \texttt{admin}. Nel caso invece si voglia provare l'app con i dati di prova, vedi \cref{section:demo}.

\subsection{Demo}\label{section:demo}
Per fornire una demo dell'applicativo, oltre al file sql per la creazione del database, abbiamo creato un altro file contenente vari dati di prova. Una volta terminata la creazione del DB, invece di effettuare l'inserimento dei dati contenuti in \texttt{required\_data\_insertion.sql} (quindi ad installazione pulita), inserire i dati contenuti nel file \texttt{demo\_data\_insertion.sql}. Il file contiene anche vari account per ogni livello di utente, di cui riportiamo username e password nella \cref{table:demo_accounts}.

\begin{table}[H]
\centering
\begin{tabular}{|l|l|l|}
\hline
\textbf{Livello} & \textbf{Username} & \textbf{Password} \\
\hline
...
\hline
\end{tabular}
\caption{Account di demo}
\label{table:demo_accounts}
\end{table}
...
All'interno dell'app è possibile effettuare il login oppure registrarsi tramite le apposite tab \textbf{Login} e \textbf{Registrati}. In particolare, a partire dalla registrazione, la password verrà criptata (utilizzando la classe BCrypt\footnote{Link alla libreria: \url{https://github.com/jeremyh/jBCrypt}}) e poi salvata sul database, in modo che nulla giri in chiaro. Una volta registrati si assume in automatico il livello \texttt{Utente base}. \\
